  ...
  return (
    <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 transition-colors duration-200">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-gray-900 dark:text-white font-semibold text-lg">Patient Vitals Input</h2>
        <div className="flex gap-2">
          <button onClick={() => setValues(NORMAL_VITALS)}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-200 dark:hover:bg-gray-700 transition-all border border-gray-200 dark:border-gray-700">
            <RotateCcw className="w-3 h-3" /> Normal
          </button>
          <button onClick={() => setValues(CRITICAL_VITALS)}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-lg bg-red-500/15 text-red-500 dark:text-red-400 hover:bg-red-500/25 transition-all border border-red-500/30">
          ...
          return (
            <div key={key} className="space-y-1.5">
              <div className="flex justify-between items-center">
                <label className="text-gray-500 dark:text-gray-400 text-xs font-medium">{label}</label>
                <span className={`text-xs px-1.5 py-0.5 rounded ${bad ? "bg-red-500/15 text-red-500 dark:text-red-400" : "bg-gray-100 dark:bg-gray-800 text-gray-500"}`}>
                  Normal: {normal} {unit}
                </span>
              </div>
              <div className="relative">
                <input
                  ...
                  className={`w-full bg-gray-50 dark:bg-gray-800 border rounded-lg px-3 py-2.5 text-gray-900 dark:text-white text-sm pr-14 focus:outline-none focus:ring-1 transition-all ${
                    bad
                      ? "border-red-400 dark:border-red-500/50 focus:ring-red-500/50 focus:border-red-500"
                      : "border-gray-200 dark:border-gray-700 focus:ring-blue-500/50 focus:border-blue-500"
                  }`}
                />
