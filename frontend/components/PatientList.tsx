...
const RISK_STYLE = {
  ...
  "—":    "bg-gray-100 dark:bg-gray-700/50 text-gray-500 border-gray-300 dark:border-gray-700",
};
  ...
  return (
    <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 transition-colors duration-200">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-gray-900 dark:text-white font-semibold">Patient Monitor</h3>
        <span className="text-xs text-gray-500 bg-gray-100 dark:bg-gray-800 px-2 py-1 rounded-full border border-gray-200 dark:border-gray-700">
      ...
      <div className="space-y-2">
        {list.map(p => {
          ...
          return (
            <div key={p.id}
              className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700/50 hover:border-gray-300 dark:hover:border-gray-600 transition-all group">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-gray-800 dark:text-gray-200 text-sm font-medium truncate">{p.name}</p>
              ...
              <div className="flex gap-1">
                <button onClick={() => scan(p)} disabled={loading === p.id}
                  className="p-1.5 rounded-lg text-gray-400 hover:text-blue-500 hover:bg-blue-500/10 transition-all disabled:opacity-50"
                  title="Quick scan">
                  {loading === p.id
                    ? <Loader2 className="w-3.5 h-3.5 animate-spin" />
                    : <ChevronRight className="w-3.5 h-3.5" />}
                </button>
                <button onClick={() => onSelect({ ...p, result: p.result })}
                  className="p-1.5 rounded-lg text-gray-400 hover:text-gray-700 dark:hover:text-white hover:bg-gray-200 dark:hover:bg-gray-700 transition-all"
