...
import { predictSepsis, generateTrendData, SAMPLE_PATIENTS } from "@/lib/api";
...
export default function Dashboard() {
  ...
  const [currentVitals, setCurrentVitals] = useState<VitalsInput | null>(null);
  ...
  const handlePredict = useCallback(async (vitals: VitalsInput) => {
    ...
    try {
      ...
      if (res.risk_level === "High") setAlert(true);
    } catch (e: any) {
  ...
  const handlePatientSelect = (p: Patient) => {
    setCurrentVitals(p.vitals);
    if (p.result) setResult(p.result);
  };

  // Summary stats
  const stats = [
    { label: "Patients Monitored", value: SAMPLE_PATIENTS.length, icon: Users,          color: "text-blue-400",    bg: "bg-blue-500/10"    },
    { label: "High Risk",          value: 2,                       icon: AlertTriangle,  color: "text-red-400",     bg: "bg-red-500/10"     },
    { label: "Predictions Today",  value: 47,                      icon: Activity,       color: "text-purple-400",  bg: "bg-purple-500/10"  },
    { label: "Avg Risk Score",     value: "34%",                   icon: TrendingUp,     color: "text-emerald-400", bg: "bg-emerald-500/10" },
  ];
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            <div className="space-y-6">
              {result ? (
                ...
                <div className="bg-gray-900 border border-gray-800 rounded-2xl p-8 flex flex-col items-center justify-center text-center gap-3 min-h-[280px]">
